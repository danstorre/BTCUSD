...
final class BinanceRemoteExchangeRateMapperTests: XCTestCase {
    ...
    private func failure(_ error: BinanceRemoteExchangeRateMapper.Error) -> ExchangeRateLoader.Result {
        .failure(error)
    }
    ...
    private func expectToMap(
        ...
        line: UInt = #line) {
            ...
            let receivedResult = BinanceRemoteExchangeRateMapper.map(response: response, data: data)
