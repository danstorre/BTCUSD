...
final class CryptoCompareRemoteExchangeRateLoaderTests: XCTestCase {
    ...
    func test_load_on200HTTPResponseWithItemData_deliversExchangeRate() throws {
        let (sut, spy) = makeSUT()
        let exchangeRate = createExchangeRate(symbol: "BTCUSD", price: 103312.60000000)
    ...
    private func makeSUT(url: URL = URL(string: "http://anyURL.com")!, file: StaticString = #filePath, line: UInt = #line) -> (sut: ExchangeRateLoader, spy: HTTPClientSpy) {
        let spy = HTTPClientSpy()
        let sut = CryptoExchangeRateRemoteExchangeRateLoader(client: spy, url: url)
